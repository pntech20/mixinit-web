...
export const getTracksAccounting = async (
  payload: GetAccountingPayload,
): Promise<Nullable<AccountingLabelItem[]>> => {
  const response = await axiosService.post(`${ACCOUNTING_BASE_URL}/tracks`, {
    ...payload.filter,
  });
...
export const removeDuplicateAccountingUploader = async ({
  labelId,
}): Promise<Nullable<AccountingLabelItem[]>> => {
  const response = await axiosService.post(
    `${ACCOUNTING_BASE_URL}/uploader/remove-duplicate`,
    {
      labelId,
    },
  );
